...
# def prepare_training_data(data, batch_user, batch_item, batch_label):
# 	return batch_user, data.user_feature_all[batch_user], batch_item, data.item_feature_all[batch_item], batch_label


def train_nar(data, model, optimizer, arch_optimizer, args):
	model.train()
	losses = []

	for batch_user, batch_item, batch_label in data.train_dataloader:
		users_train, users_feat_train, items_train, items_feat_train, labels_train = prepare_training_data(data, batch_user, batch_item, batch_label)
		#print("users train: {}".format(users_train))
		#print("users_feat_train train: {}".format(users_feat_train))
		#print("items_train train: {}".format(items_train))
		#print("items_feat_train train: {}".format(items_feat_train))
		#print("labels_train train: {}".format(labels_train))
		users_train = users_train.cuda()
		users_feat_train = users_feat_train.cuda()
		items_train = items_train.cuda()
		items_feat_train = items_feat_train.cuda()
		labels_train = labels_train.cuda()

		#users_valid, items_valid, labels_valid = next(iter(valid_queue))
		users_valid = users_train.cuda()
		users_feat_valid = users_feat_train.cuda()
		items_valid = items_train.cuda()
		items_feat_valid = items_feat_train.cuda()
		labels_valid = labels_train.cuda()

		if args.mode == 'sif-no-auto':
			loss_valid = model.step(users_train, users_feat_train, items_train, items_feat_train, labels_train, users_train,
									users_feat_train, items_train, items_feat_train, labels_train, args.lr, arch_optimizer, args.unrolled)
		else:
			loss_valid = model.step(users_train, users_feat_train, items_train, items_feat_train, labels_train, users_valid,
									users_feat_valid, items_valid, items_feat_valid, labels_valid, args.lr, arch_optimizer, args.unrolled)
		optimizer.zero_grad()
		arch_optimizer.zero_grad()

		model.binarize()
		inferences, regs = model(users_train, users_feat_train, items_train, items_feat_train)
...
def prepare_training_data(data, batch_user, batch_item):
	# 512, same with batch size
	#print("size of pos item:{}, content:{}".format(batch_item.shape, batch_item))
	batch_neg_item = torch.LongTensor(random.sample(range(data.item_num), batch_item.shape[0]))
	#print("size of neg item:{}, content:{}".format(batch_neg_item.shape, batch_neg_item))
	return (batch_user, data.user_feature_all[batch_user], batch_item, data.item_feature_all[batch_item],
			batch_neg_item, data.item_feature_all[batch_neg_item])


def train_nar_ce(data, model, optimizer, arch_optimizer, args):
	model.train()
	losses = []

	for batch_user, batch_item in data.train_dataloader:
		users_train, users_feat_train, pos_items_train, pos_items_feat_train, neg_items_train, neg_items_feat_train = prepare_training_data(data, batch_user, batch_item)
		'''
		print("users train: {}".format(users_train))
		print("users_feat_train train: {}".format(users_feat_train))
		print("items_train train: {}".format(pos_items_train))
		print("items_feat_train train: {}".format(pos_items_feat_train))
		print("neg items_train train: {}".format(neg_items_train))
		print("neg items_feat_train train: {}".format(neg_items_feat_train))
		'''
		users_train = users_train.cuda()
		users_feat_train = users_feat_train.cuda()
		pos_items_train = pos_items_train.cuda()
		pos_items_feat_train = pos_items_feat_train.cuda()
		neg_items_train = neg_items_train.cuda()
		neg_items_feat_train = neg_items_feat_train.cuda()


		users_valid, items_valid = next(iter(data.val_dataloader))
		users_valid, users_feat_valid, pos_items_valid, pos_items_feat_valid, neg_items_valid, neg_items_feat_valid = prepare_training_data(data, users_valid, items_valid)
		users_valid = users_valid.cuda()
		users_feat_valid = users_feat_valid.cuda()
		pos_items_valid = pos_items_valid.cuda()
		pos_items_feat_valid = pos_items_feat_valid.cuda()
		neg_items_valid = neg_items_valid.cuda()
		neg_items_feat_valid = neg_items_feat_valid.cuda()


		if args.mode == 'sif-no-auto':
			loss_valid = model.step(users_train, users_feat_train, pos_items_train, pos_items_feat_train, neg_items_train, neg_items_feat_train,
									users_train, users_feat_train, pos_items_train, pos_items_feat_train, neg_items_train, neg_items_feat_train,
									args.lr, arch_optimizer, args.unrolled)
		else:
			loss_valid = model.step(users_train, users_feat_train, pos_items_train, pos_items_feat_train, neg_items_train, neg_items_feat_train,
		...
		#inferences, regs = model(users_train, users_feat_train, items_train, items_feat_train)
		#loss = model.compute_loss(inferences, labels_train, regs)
		loss = model.compute_loss(users_train, users_feat_train, pos_items_train, pos_items_feat_train, neg_items_train, neg_items_feat_train)
		loss.backward()
		model.recover()
		nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
		optimizer.step()
