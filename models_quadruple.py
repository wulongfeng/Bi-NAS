...
PRIMITIVES_QUADRUPLE = ['plus_concat_plus', 'plus_concat_multiply', 'plus_concat_concat',
                     'multiply_concat_plus', 'multiply_concat_multiply', 'multiply_concat_concat',
                     'concat_concat_plus', 'concat_concat_multiply', 'concat_concat_concat'
                     ]
PRIMITIVES_NAS = [0, 2, 4, 8, 16]

def ops_quadruple(quadruple, p, q, r, s):
    ops = quadruple.split('_')
    assert len(ops) == 3
    return OPS[ops[1]](OPS[ops[0]](p, q), OPS[ops[2]](r, s))

def _concat(xs):
    return torch.cat([x.view(-1) for x in xs])


def MixedQuadruple(embedding_p, embedding_q, embedding_r, embedding_s, weights, FC):
    return torch.sum(torch.stack([w * fc(ops_quadruple(primitive, embedding_p, embedding_q, embedding_r, embedding_s)) \
                                  for w,primitive,fc in zip(weights,PRIMITIVES_QUADRUPLE,FC)]), 0)


class Virtue_Quadruple(nn.Module):
    def __init__(self, num_users, num_items, embedding_dim, reg, w2v, feat_dim, att):
        super(Virtue_Quadruple, self).__init__()
...
class NCF_Triple(Virtue_Quadruple):
...
class DeepWide_Triple(Virtue_Quadruple):
...
class NAS_Triple(Virtue_Quadruple):
...
class AutoNeural_Triple(Virtue_Quadruple):
...
class Network_NAR_Quadruple(Virtue_Quadruple):
    def __init__(self, num_users, num_items, embedding_dim, reg, w2v, feat_dim, att):
        super(Network_NAR_Quadruple, self).__init__(num_users, num_items, embedding_dim, reg, w2v, feat_dim, att)
        self.num_users = num_users
        self.num_items = num_items
        self.embedding_dim = embedding_dim
        self.reg = reg
        ...
        for primitive in PRIMITIVES_QUADRUPLE:
            if primitive == 'concat_concat_concat':
                self._FC.append(nn.Linear(4*embedding_dim, 1, bias=False))
            elif 'concat_concat' in primitive:
                self._FC.append(nn.Linear(3*embedding_dim, 1, bias=False))
    ...
    def _initialize_alphas(self):
        ...
        self._arch_parameters['quadruple'] = Variable(torch.ones(len(PRIMITIVES_QUADRUPLE), dtype=torch.float, device='cuda') /2, requires_grad=True)
        self._arch_parameters['quadruple'].data.add_(
            torch.randn_like(self._arch_parameters['quadruple'])*1e-3)


    def arch_parameters(self):
        return list(self._arch_parameters['mlp']['p'].parameters()) + \
               list(self._arch_parameters['mlp']['q'].parameters()) + \
               [self._arch_parameters['quadruple']]

    def new(self):
        model_new = Network_NAR_Quadruple(self.num_users, self.num_items, self.embedding_dim, self.reg, self.w2v, self.feat_dim, self.att).cuda()
    ...
    def clip(self):
        m = nn.Hardtanh(0, 1)
        self._arch_parameters['quadruple'].data = m(self._arch_parameters['quadruple'])

    def binarize(self):
        self._cache = self._arch_parameters['quadruple'].clone()
        max_index = self._arch_parameters['quadruple'].argmax().item()
        for i in range(self._arch_parameters['quadruple'].size(0)):
            if i == max_index:
                self._arch_parameters['quadruple'].data[i] = 1.0
            else:
                self._arch_parameters['quadruple'].data[i] = 0.0

    def recover(self):
        self._arch_parameters['quadruple'].data = self._cache
        del self._cache
    ...
    def forward(self, user, user_feat, item, item_feat):
        ...
        for i in range(len(PRIMITIVES_QUADRUPLE)):
        ...
        output = MixedQuadruple(user_word_embedding, item_word_embedding, users_embedding, items_embedding,
                                self._arch_parameters['quadruple'], self._FC)
    ...
    def genotype(self):
        genotype = PRIMITIVES_QUADRUPLE[self._arch_parameters['quadruple'].argmax().cpu().numpy()]
        genotype_p = F.softmax(self._arch_parameters['quadruple'], dim=-1)
